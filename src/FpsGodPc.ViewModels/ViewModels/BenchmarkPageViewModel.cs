    ...
    public async Task RunBenchmark()
    {
        ...
        try
        {
            ...
            if (!IsHeavenReady)
            {
                ...
                if (!ensure.Success)
                {
                    DialogService.Alert(ensure.Message, "FPS Optimize GOD PC", DialogKind.Warning);
                    IsRunning = false;
                    return;
                }
            }
            ...
            if (!result.Success)
            {
                DialogService.Alert(result.Message, "FPS Optimize GOD PC", DialogKind.Warning);
            }
        ...
        catch (Exception ex)
        {
            ...
            DialogService.Alert(ex.Message, "FPS Optimize GOD PC", DialogKind.Error);
        }
        finally
        {
            IsRunning = false;
